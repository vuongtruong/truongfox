...
import static com.younetco.ionicon.Iconify.IconValue;
...
class Utils {
    ...
    static File resourceToFile(Context context, String resourceName) throws IOException {
        ...
        if (!f.exists())
            if (!f.mkdirs()) {
                Log.e(com.younetco.ionicon.Iconify.TAG, "Font folder creation failed");
        ...
        try {
            inputStream = com.younetco.ionicon.Iconify.class.getClassLoader().getResourceAsStream(resourceName);
    ...
    public static StringBuilder replaceIcons(StringBuilder text) {
        int startIndex = text.indexOf("{ion");
        if (startIndex == -1) {
            return text;
        }
        ...
        try {
            ...
            Log.w(com.younetco.ionicon.Iconify.TAG, "Wrong icon name: " + iconString);
            return text;
        }
    }
}
