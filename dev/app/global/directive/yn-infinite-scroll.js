/**
 * @ngdoc directive
 * @name ynInfiniteScroll
 * @module ionic
 * @parent ionic.directive:ionContent, ionic.directive:ionScroll
 * @restrict E
 *
 * @description
 * The ynInfiniteScroll directive allows you to call a function whenever
        ...
        return {
            ...
            controller: ['$scope', '$attrs',
                function($scope, $attrs) {
                    ...
                    this.getDistanceInPx = function() {
                        var distance = ($attrs.distance || '2.5%').trim();
                        var isPercent = distance.indexOf('%') !== -1;
                        if (isPercent) {
                            return window.outerHeight * parseFloat(distance) / 100;
                        }
            ...
            link: function($scope, $element, $attrs, ctrls) {
                ...
                window.onscroll = checkBounds;
                // scrollCtrl.$element.on('scroll', checkBounds);

                function checkInfiniteBounds() {
                    if (infiniteScrollCtrl.isLoading) return;
                    var outerHeight = window.myappOuterHeight ? window.myappOuterHeight : window.outerHeight;
                    if (outerHeight + window.scrollY + 25 >= document.body.offsetHeight - infiniteScrollCtrl.getDistanceInPx()) {
                        onInfinite();
                    }
