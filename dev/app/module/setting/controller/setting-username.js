define([
    ...
    return function($scope, $injector, $state, $http2, $modal, gettextCatalog) {
        ...
        $scope.fetchData = function() {
            ...
            $http2.post('setting/form_user_name').success($scope.fetchDataSuccess).error($scope.fetchDataError).
            finally(function() {
        ...
        $scope.fetchDataSuccess = function(data) {
            ...
            $scope.form = data;
            $scope.formData.user_name = $scope.form.user_name;
        ...
        $scope.onSave = function() {
            ...
            $http2.post('setting/update_user_name', $scope.formData).success($scope.saveSuccess).error($scope.saveError).
            finally(function() {
        ...
        $scope.isValidData = function(bAlert) {
            ...
            if (!$scope.formData.user_name) {
                bAlert && $modal.alert(gettextCatalog.getString('Provide a user name.'));
                return false;
            }
